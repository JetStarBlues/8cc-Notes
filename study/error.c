...
//#include <stdlib.h>
#include <stdio.h>   // fileno, stderr, fprintf, vfprintf, exit
#include <unistd.h>  // isatty
#include "8cc.h"     // File type, Token type
...
{
	fprintf(  stderr, isatty( fileno( stderr ) ) ? "\e[1;31m[%s]\e[0m " : "[%s] ", label );  // if supported, color 'label'
	fprintf(  stderr, "%s: %s: ", line, pos );
	vfprintf( stderr, fmt, args );  // variable number of args
	fprintf(  stderr, "\n" );
}
