...
{
	Vector *r = malloc( sizeof( Vector ) );  // how do we know sizeof Vector?

	size = roundup( size );

	if ( size > 0 )
	{
		r->body = malloc( size * sizeof( void * ) );  // ?
	}
...
static void extend ( Vector *vec, int delta )
{
	...
	void *newbody = malloc( nelem * sizeof( void * ) );
